...
def addroad(a_s,street,str_co,c_m_dict):
    ...
    if (street in c_m_dict.keys()):
        print('Error: Entered Street data already exist')
        return
    for i in range(0,len(str_co)-1):
        x=str_co[i]
        y=str_co[i+1]
        if x[-1] == ')' and y[-1] == ')' and x[0]=='(' and y[0] == '(':
            new_street.append(x + ' -->' + y)
        else:
            print ('Error: Please enter street co-ordinates using brackets')
    c_m_dict[street]= new_street
    return c_m_dict

# Modifying Street co-ordinates

def modroad(a_s,street,str_co,c_m_dict):
    new_street=[]
    if (street not in c_m_dict.keys()):
        print('Error: Street does not exist : Please enter valid data')
        return
    for i in range(0,len(str_co)-1):
        x=str_co[i]
        y=str_co[i+1]
...
# Removing a Street and its Co-ordinates
...
# Generating a graph with command 'gg' 
...
# Finding Intersection between 2 lines function
...
### Function to calculate edges of lines
